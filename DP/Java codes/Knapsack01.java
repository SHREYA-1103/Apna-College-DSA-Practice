...
public class Knapsack01 {
    ...
    public static int maxProfit_rec(int val[], int wt[], int cap, int n){
        ...
        if(wt[n-1] <= cap){ // valid item - can be included or excluded
            return Math.max(val[n-1] + maxProfit_rec(val, wt, cap-wt[n-1], n-1), maxProfit_rec(val, wt, cap, n-1));
        }
    ...
    public static int maxProfit_memo(int val[], int wt[], int cap, int n, int dp[][]){
        ...
        if(wt[n-1] <= cap){ // valid item - can be included or excluded
            return dp[n][cap] = Math.max(val[n-1] + maxProfit_memo(val, wt, cap-wt[n-1], n-1, dp), maxProfit_memo(val, wt, cap, n-1, dp));
        }
